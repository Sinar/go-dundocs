...
func Test_detectHansardType(t *testing.T) {
	...
	tests := []struct {
		...
		{"sad #1", args{PDFPage{PDFTxtSameLines: []string{"", ""}}}, -1},
		{"happy #1", args{PDFPage{PDFTxtSameLines: []string{"", ""}}}, HANSARD_WRITTEN},
		{"happy #2", args{PDFPage{PDFTxtSameLines: []string{"", ""}}}, HANSARD_SPOKEN},
		{"weird #1", args{PDFPage{PDFTxtSameLines: []string{"", ""}}}, HANSARD_DEBATE},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
